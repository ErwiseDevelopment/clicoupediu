   ...
   public function salvar() {
        ...
        try {
            ...
            $dados = [
                ...
                'endereco'         => $_POST['endereco_entrega'] ?? '', // Ajustado nome do campo conforme form
                'numero'           => $_POST['numero'] ?? '',
                'bairro'           => $_POST['bairro'] ?? '',
            ...
            // Recalcula valor dos produtos para segurança
            $valorProd = 0;
            ...
            // =================================================================
            // LÓGICA ESPECIAL PARA MESA: TENTAR AGRUPAR
            // =================================================================
            if ($dados['tipo_entrega'] === 'salao' && !empty($dados['sessao_id']) && empty($dados['pedido_id'])) {
                
                // 1. Busca Pedido Aberto na mesma sessão (evita criar novo)
                // Se tiver 'participante_id', tenta achar o pedido DELE. Se não, qualquer um da sessão (comanda única).
                ...
                if (!empty($dados['participante_id'])) {
                    // Verifica se a sessão é individual antes de forçar o ID? 
                    // Para simplificar: se veio participante_id, tentamos usar o pedido dele.
                ...
                if ($pedidoExistente) {
                    ...
                    exit; // Encerra aqui para não criar outro pedido
                }
            }
            // =================================================================

            // Se não for mesa ou não tiver pedido aberto, segue o fluxo normal (Cria novo)
            $model = new Pedido();
            if ($dados['pedido_id']) {
    ...
    header('Content-Type: application/json');
    $hash = $_GET['h'] ?? ''; // O hash que vem da URL
    
    $db = Database::connect();
    
    // 1. Busca a mesa pelo Hash
    ...
    // 2. Verifica se tem sessão aberta
    ...
    if(!$sessao) {
        // MESA LIVRE: Front deve mostrar tela "Abrir Mesa" (Pedir Nome + Tipo Divisão)
        echo json_encode(['ok' => true, 'estado' => 'LIVRE', 'mesa' => $mesa]);
    } else {
        // MESA OCUPADA: Front deve mostrar tela "Entrar na Mesa" (Pedir Nome)
...
public function abrirOuEntrarMesa() {
    ...
    $acao = $_POST['acao']; // 'abrir' ou 'entrar'
    ...
    try {
        ...
        // Retorna dados para o LocalStorage do celular do cliente
        echo json_encode([
            'ok' => true, 
            'sessao_id' => $sessaoId, 
  ...
  public function imprimir() {
        ...
        // 1. Busca o pedido para checar o TIPO
        ...
        // Se for MESA, redireciona para o controller certo e evita erro
        ...
        // Se for DELIVERY, continua normal...
  ...
  public function mudarStatus() {
        ...
        try {
            ...
            // ---------------------------------------------------------
            // 2. LÓGICA KDS (NOVA): Baixa automática nos Itens
            // ---------------------------------------------------------
            // Se o pedido foi para 'entrega' ou 'finalizado', marca todos os itens como 'entregue'
            if ($novoStatus == 'entrega' || $novoStatus == 'finalizado') {
                $db->prepare("UPDATE pedido_itens SET status_item = 'entregue' WHERE pedido_id = ?")
                   ->execute([$id]);
            }
            // Opcional: Se voltou para preparo, reseta itens (útil para correções)
            elseif ($novoStatus == 'preparo') {
                 $db->prepare("UPDATE pedido_itens SET status_item = 'preparo' WHERE pedido_id = ? AND status_item = 'fila'")
                   ->execute([$id]);
            }

            // ---------------------------------------------------------
            // 3. LÓGICA FINANCEIRA: Só lança se for FINALIZADO
            // ---------------------------------------------------------
            if ($novoStatus == 'finalizado') {
                
                // Busca dados completos do pedido para lançar no financeiro
                $stmt = $db->prepare("SELECT * FROM pedidos WHERE id = ? AND empresa_id = ?");
                $stmt->execute([$id, $empresaId]);
                $pedido = $stmt->fetch(\PDO::FETCH_ASSOC);
                ...
                if ($pedido) {
                    ...
                    if ($stmtCheck->rowCount() == 0) {
                        ...
                        // Verifica se 'forma_pagamento' contém 'fiado' (insensitive case) ou é igual
    ...
    // --- API: VERIFICA SE TEM PEDIDO NOVO PARA IMPRIMIR ---
    ...
    public function verificarFilaImpressao() {
        ...
        // CORREÇÃO SQL: 
        // 1. impresso = 0 (Nunca impresso)
        // 2. Status: Somente 'novo', 'analise' ou 'preparo'. 
        //    (Ignora 'entrega', 'finalizado', 'cancelado')
        // 3. Data: Somente últimas 12 horas (evita imprimir coisas velhas)
        
        $sql = "SELECT id, tipo_entrega, sessao_id 
                FROM pedidos 
                WHERE empresa_id = ? 
                AND impresso = 0 
                AND status IN ('novo', 'analise', 'preparo') 
    ...
    // 2. Marca o pedido como impresso (chamado logo após a impressão)
  ...
  public function monitorUnificado() {
        ...
        foreach($todosPedidos as &$p) {
            // SQL BLINDADO: Usa LEFT JOIN e traz itens mesmo sem produto ou status NULL
    ...
    // --- NOVA FUNÇÃO: IMPRIMIR COZINHA (ADICIONE NO FINAL DA CLASSE) ---
    public function imprimirCozinha() {
        ...
        // 2. Busca Itens (CORREÇÃO AQUI: Aceita status 'fila', 'preparo' ou NULL. Só ignora 'cancelado')
    ...
    public function apiLoginSpooler() {
        ...
        // Recebe Email e Senha do Python
        $email = $_POST['email'] ?? '';
        $senha = $_POST['senha'] ?? '';
        ...
        if ($user && password_verify($senha, $user['senha'])) {
            ...
            // Busca dados da empresa para mostrar no app
            ...
            echo json_encode([
                ...
                'nome_loja' => $empresa['nome_fantasia']
            ]);
        } else {
    ...
    public function apiVerificarFila() {
        // Define que a resposta é JSON (importante para o Python ler)
        header('Content-Type: application/json');
        
        // O Robô manda o ID da empresa e o Token (se tiver login)
        ...
        // BUSCA PEDIDOS NA FILA
        // Regra: Status 'novo' ou 'preparo' E que a coluna 'impresso' seja 0
        $sql = "SELECT p.id, p.cliente_nome, p.tipo_entrega, p.status, p.created_at, 
                       p.sessao_id, p.bairro, m.numero as num_mesa
                FROM pedidos p
                LEFT JOIN mesa_sessoes s ON p.sessao_id = s.id
                ...
                ORDER BY p.id ASC 
                LIMIT 1"; // Pega um por vez para não travar a impressora
        ...
        if ($pedido) {
            // Se achou o pedido, agora busca os ITENS dele
            // Usamos LEFT JOIN para trazer o nome do produto mesmo se ele foi deletado
            ...
            // Busca os COMPLEMENTOS (Adicionais) de cada item
            foreach($itens as &$i) {
                // Se sua tabela de complementos for simples (só string):
                /* $stmtAdd = $db->prepare("SELECT nome FROM pedido_item_complementos WHERE pedido_item_id = ?");
                $stmtAdd->execute([$i['id']]);
                $i['complementos'] = $stmtAdd->fetchAll(\PDO::FETCH_COLUMN);
                */
                // Se não tiver complementos ainda, deixa array vazio para não dar erro no Python
                $i['complementos'] = []; 
            }

            $pedido['itens'] = $itens;
            
            // Responde SIM para o Python
            echo json_encode(['tem_pedido' => true, 'dados' => $pedido]);
        } else {
            // Responde NÃO (o Python vai esperar 5seg e tentar de novo)
            echo json_encode(['tem_pedido' => false]);
        }
        exit;
    }

    // 2. O Robô avisa: "Imprimi com sucesso!"
    public function apiConfirmarImpressao() {
        ...
        if ($pedidoId) {
            $db = \App\Core\Database::connect();
            // Marca impresso = 1 para não sair de novo
    ...
    // 3. Botão "Reimprimir" (Para usar no Monitor KDS se o papel acabar)
    public function resetarImpressao() {
        $this->verificarLogin(); // Só admin logado pode fazer isso
        $id = $_POST['id'] ?? 0;
        
        if($id) {
            $db = \App\Core\Database::connect();
            // Volta para 0, aí o Robô pega de novo na próxima varredura
